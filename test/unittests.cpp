...
void TestSignatures()
{
    ...
    }
    {
        ...
        if (sk1 != sk2)
            throw invalid_argument("sk1 must equal sk2!");
        if (sk1 == sk3)
            throw invalid_argument("sk1 must NOT equal sk3!");
        if (!pk1.equal(pk2))
            throw invalid_argument("pk1 must equal pk2!");
        if (!pk2.equal(pk3))
            throw invalid_argument("pk2 must equal pk3!");
        if (pk1.equal(pk4))
            throw invalid_argument("pk1 must NOT equal pk4!");
        if (!sig1.equal(sig2))
            throw invalid_argument("sig1 must equal sig2!");
        if (!sig2.equal(sig3))
            throw invalid_argument("sig2 must equal sig3!");
        if (sig3.equal(sig4))
            throw invalid_argument("sig3 must NOT equal sig4!");

        if (pk1.toCompressedBytesBE() != pk2.toCompressedBytesBE())
            throw invalid_argument("serialized pk1 must equal serialized pk2!");
        if (sig1.toCompressedBytesBE() != sig2.toCompressedBytesBE())
            throw invalid_argument("serialized sig1 must equal serialized sig2!");
    }
    {
        vector<uint8_t> message1 = {1, 65, 254, 88, 90, 45, 22};
...
void TestAggregateSKs()
{
    ...
    if (aggSk != aggSkAlt)
        throw invalid_argument("aggSk != aggSkAlt");

    const g1 aggPubKey = pk1.add(pk2);
    if (!aggPubKey.equal(public_key(aggSk)))
        throw invalid_argument("aggPubKey != public_key(aggSk)");
    ...
    if (!aggSig.equal(aggSig2))
        throw invalid_argument("aggSig != aggSig2");

    // Verify as a single G2Element
    if (!verify(aggPubKey, message, aggSig))
        throw invalid_argument("aggSig verify failed");
    if (!verify(aggPubKey, message, aggSig2))
        throw invalid_argument("aggSig2 verify failed");

    // Verify aggregate with both keys (Fails since not distinct)
    if (aggregate_verify(std::array{pk1, pk2}, vector<vector<uint8_t>>{message, message}, aggSig, true))
        throw invalid_argument("aggregate verify of aggSig must fail");
    if (aggregate_verify(std::array{pk1, pk2}, vector<vector<uint8_t>>{message, message}, aggSig2, true))
        throw invalid_argument("aggregate verify of aggSig2 must fail");
    ...
    if (!aggSigFinal.equal(aggSigAlt))
        throw invalid_argument("aggSigFinal != aggSigAlt");
    if (!aggSigFinal.equal(aggSigAlt2))
        throw invalid_argument("aggSigFinal != aggSigAlt2");
    ...
    if (skFinal != skFinalAlt)
        throw invalid_argument("skFinal != skFinalAlt");
    if (skFinal == aggSk)
        throw invalid_argument("skFinal == aggSk");

    g1 pkFinal = aggPubKey.add(pk2);
    g1 pkFinalAlt = pk2.add(pk1).add(pk2);
    if (!pkFinal.equal(pkFinalAlt))
        throw invalid_argument("pkFinal != pkFinalAlt");
    if (pkFinal.equal(aggPubKey))
        throw invalid_argument("pkFinal == aggPubKey");

    // Verify with aggPubKey (since we have multiple messages)
    if (!aggregate_verify(std::array{aggPubKey, pk2}, vector<vector<uint8_t>>{message, message2}, aggSigFinal, true))
        throw invalid_argument("verify with aggPubKey failed");
}
...
{
    {
        ...
        // Wrong sig
        if (verify(pk1, msg1, sig2))
            throw invalid_argument("must fail: wrong sig");
        // Wrong msg
        if (verify(pk1, msg2, sig1))
            throw invalid_argument("must fail: wrong msg");
        // Wrong pk
        if (verify(pk2, msg1, sig1))
            throw invalid_argument("must fail: wrong pk");

        g2 aggsig = aggregate_signatures(std::array{sig1, sig2});
        if (!aggregate_verify(std::array{pk1, pk2}, msgs, aggsig))
            throw invalid_argument("aggregate_verify failed");

        // PopVerify
        g2 proof1 = pop_prove(sk1);
        if (!pop_verify(pk1, proof1))
            throw invalid_argument("pop_verify failed");
        ...
        if (!pop_fast_aggregate_verify(std::array{pk1, pk2}, msg1, aggsig_same))
            throw invalid_argument("pop_fast_aggregate_verify failed");
    }
    {
    ...
    }
    {
        ...
        if (!pop_verify(pk1, pop1))
            throw invalid_argument("pop_verify 1 failed");
        if (!pop_verify(pk2, pop2))
            throw invalid_argument("pop_verify 2 failed");
        if (!pop_verify(pk3, pop3))
            throw invalid_argument("pop_verify 3 failed");
        g2 popSigAgg = aggregate_signatures(std::array{popSig1, popSig2, popSig3});

        if (!pop_fast_aggregate_verify(std::array{pk1, pk2, pk3}, message, popSigAgg))
            throw invalid_argument("pop_fast_aggregate_verify failed");

        // Aggregate public key, indistinguishable from a single public key
        g1 popAggPk = pk1.add(pk2).add(pk3);
        if (!verify(popAggPk, message, popSigAgg))
            throw invalid_argument("verify popSigAgg failed");

        // Aggregate private keys
        array<uint64_t, 4> aggSk = aggregate_secret_keys(std::array{sk1, sk2, sk3});
        if (!sign(aggSk, message).equal(popSigAgg))
            throw invalid_argument("sign(aggSk, message) != popSigAgg");
    }
}
