...
// If z is equal to one the point is considered as in affine form.
    ...
    public:
        ...
        template<size_t N>
        g1 scale(const std::array<uint64_t, N> &s) const;
        ...
        static g1 weightedSum(tcb::span<const g1> points, tcb::span<const std::array<uint64_t, 4>> scalars,
                              const std::function<void()> &yield = std::function<void()>());
...
// If z is equal to one the point is considered as in affine form.
    ...
    public:
        ...
        template<size_t N>
        g2 scale(const std::array<uint64_t, N> &s) const;
        ...
        static g2 weightedSum(tcb::span<const g2> points, tcb::span<const std::array<uint64_t, 4>> scalars,
                              const std::function<void()> &yield = std::function<void()>());
